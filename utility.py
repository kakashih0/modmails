...
class Kakashi MailHelpCommand(commands.HelpCommand):
    ...
    async def send_bot_help(self, mapping):
        ...
        # always come first
        default_cogs = [bot.get_cog("Kakashi Mail"), bot.get_cog("Utility"), bot.get_cog("Plugins")]
...
class Utility(commands.Cog):
    ...
    def __init__(self, bot):
        ...
        self.bot.help_command = Kakashi MailHelpCommand(
            verify_checks=False,
            command_attrs={
    ...
    async def changelog(self, ctx, version: str.lower = ""):
        """Shows the changelog of the Kakashi Mail."""
    ...
    async def about(self, ctx):
        ...
        embed.set_author(
            name="Kakashi Mail - About",
            icon_url=self.bot.user.avatar_url,
            url="https://discord.gg/d6VqKqs",
        )
        embed.set_thumbnail(url=self.bot.user.avatar_url)

        desc = "This bot was created by Kakashi#2861"
        desc += "If you would like a custom version, message me on discord"
        desc += "I offer both free and paid plans"
        embed.description = desc

        embed.add_field(name="Uptime", value=self.bot.uptime)
        embed.add_field(name="Latency", value=f"{self.bot.latency * 1000:.2f} ms")
        embed.add_field(name="Version", value=f"`{self.bot.version}`")
        embed.add_field(name="Authors", value="`Kakashi#2861`")
        ...
        embed.add_field(
            name="Want Kakashi Mail in Your Server?",
            value="Message me for the invite link!",
            inline=False,
        )

        embed.add_field(
            name="Support the Developers",
            value="This bot is completely free for everyone however we also offer custom versions of the bot for cheap. Just message Kakashi#2861",
            inline=False,
        )
    ...
    async def sponsors(self, ctx):
        ...
        resp = await self.bot.session.get(
            "https://raw.githubusercontent.com/Kakashi#2861/Kakashi Mail/master/SPONSORS.json"
        )
    ...
    async def set_presence(self, *, status=None, activity_type=None, activity_message=None):
        ...
        if activity_type is not None and not activity_message:
            logger.warning(
                'No activity message found whilst activity is provided, defaults to "Kakashi Mail".'
            )
            activity_message = "Kakashi Mail"
    ...
    async def permissions(self, ctx):
        """
        Set the permissions for Kakashi Mail commands.
        ...
        Acceptable permission levels are:
            ...
            - **Supporter** [2] (access to core Kakashi Mail supporting functions)
        ...
        Note: You will still have to manually give/take permission to the Kakashi Mail
        ...
        if type_ == "command":
            ...
            if level > PermissionLevel.REGULAR:
                if value == -1:
                    ...
                    key = self.bot.Kakashi Mail_guild.get_member(value)
                if key is not None:
    ...
    async def permissions_remove(
        ...
        *,
        ...
        if type_ == "level":
            if level > PermissionLevel.REGULAR:
                if value == -1:
                    logger.info("Denying @everyone access to Kakashi Mail category.")
                    await self.bot.main_category.set_permissions(
                        self.bot.Kakashi Mail_guild.default_role, read_messages=False
                    )
                elif isinstance(user_or_role, discord.Role):
                    logger.info("Denying %s access to Kakashi Mail category.", user_or_role.name)
                    await self.bot.main_category.set_permissions(user_or_role, overwrite=None)
                else:
                    member = self.bot.Kakashi Mail_guild.get_member(value)
                    if member is not None and member != self.bot.Kakashi Mail_guild.me:
                        logger.info("Denying %s access to Kakashi Mail category.", member.name)
    ...
    async def oauth_whitelist(self, ctx, target: Union[discord.Role, utils.User]):
        ...
        if not hasattr(target, "mention"):
            target = self.bot.get_user(target.id) or self.bot.Kakashi Mail_guild.get_role(target.id)
    ...
    async def oauth_show(self, ctx):
        ...
        for id_ in whitelisted:
            ...
            if user:
                users.append(user)
            role = self.bot.Kakashi Mail_guild.get_role(id_)
            if role:
                roles.append(role)
