...
class Kakashi Mail(commands.Cog):
    """Commands directly related to Kakashi Mail functionality."""
    ...
    async def setup(self, ctx):
        """
        Sets up a server for Kakashi Mail.

        You only need to run this command
        once after configuring Kakashi Mail.
        """

        if ctx.guild != self.bot.Kakashi Mail_guild:
            return await ctx.send(
                f"You can only setup in the Kakashi Mail guild: {self.bot.Kakashi Mail_guild}."
            )

        if self.bot.main_category is not None:
            logger.debug("Can't re-setup server, main_category is found.")
            return await ctx.send(f"{self.bot.Kakashi Mail_guild} is already set up.")

        if self.bot.Kakashi Mail_guild is None:
            embed = discord.Embed(
                title="Error",
                description="Kakashi Mail functioning guild not found.",
                color=self.bot.error_color,
            )
            return await ctx.send(embed=embed)

        overwrites = {
            self.bot.Kakashi Mail_guild.default_role: discord.PermissionOverwrite(read_messages=False),
            self.bot.Kakashi Mail_guild.me: discord.PermissionOverwrite(read_messages=True),
        }
        ...
        for level in PermissionLevel:
            ...
            for perm in permissions:
                perm = int(perm)
                if perm == -1:
                    key = self.bot.Kakashi Mail_guild.default_role
                else:
                    key = self.bot.Kakashi Mail_guild.get_member(perm)
                    if key is None:
                        key = self.bot.Kakashi Mail_guild.get_role(perm)
                if key is not None:
                    logger.info("Granting %s access to Kakashi Mail category.", key.name)
                    overwrites[key] = discord.PermissionOverwrite(read_messages=True)

        category = await self.bot.Kakashi Mail_guild.create_category(
            name="Kakashi Mail", overwrites=overwrites
        )

        await category.edit(position=0)

        log_channel = await self.bot.Kakashi Mail_guild.create_text_channel(
        ...
        embed.add_field(
            name="Thank you for using Kakashi Mail!",
        ...
        await ctx.send(
            ...
            "or users the ability to use Kakashi Mail.\n\n"
    ...
    async def nsfw(self, ctx):
        """Flags a Kakashi Mail thread as NSFW (not safe for work)."""
    ...
    async def sfw(self, ctx):
        """Flags a Kakashi Mail thread as SFW (safe for work)."""
    ...
    async def logs(self, ctx, *, user: User = None):
        """
        Get previous Kakashi Mail thread logs of a member.
    ...
    async def reply(self, ctx, *, msg: str = ""):
        """
        Reply to a Kakashi Mail thread.
    ...
    async def freply(self, ctx, *, msg: str = ""):
        """
        Reply to a Kakashi Mail thread with variables.
        ...
        *,
        ...
        if exists:
            ...
            if self.bot.config["dm_disabled"] >= 1:
                logger.info("Contacting user %s when Kakashi Mail DM is disabled.", user)
    ...
    async def block(self, ctx, user: Optional[User] = None, *, after: UserFriendlyTime = None):
        """
        Block a user from using Kakashi Mail.
    ...
    async def unblock(self, ctx, *, user: User = None):
        """
        Unblock a user from using Kakashi Mail.
    ...
    async def repair(self, ctx):
        ...
        if thread is not None:
            ...
            await ctx.channel.edit(reason="Fix broken Kakashi Mail thread", topic=f"User ID: {user_id}")
            return await self.bot.add_reaction(ctx.message, sent_emoji)
            ...
            if (
                ...
                if user_id != -1:
                    ...
                    await ctx.channel.edit(
                        reason="Fix broken Kakashi Mail thread", topic=f"User ID: {user_id}"
                    )
                    return await self.bot.add_reaction(ctx.message, sent_emoji)
        ...
        if m is not None:
            ...
            if len(users) == 1:
                ...
                name = format_channel_name(
                    user, self.bot.Kakashi Mail_guild, exclude_channel=ctx.channel
                )
                ...
                await ctx.channel.edit(
                    reason="Fix broken Kakashi Mail thread", name=name, topic=f"User ID: {user.id}"
                )
                return await self.bot.add_reaction(ctx.message, sent_emoji)
    ...
    async def enable(self, ctx):
        """
        Re-enables DM functionalities of Kakashi Mail.

        Undo's the `{prefix}disable` command, all DM will be relayed after running this command.
        """
        embed = discord.Embed(
            title="Success",
            description="Kakashi Mail will now accept all DM messages.",
            color=self.bot.main_color,
        )
    ...
    async def disable(self, ctx):
        """
        Disable partial or full Kakashi Mail thread functions.

        To stop all new threads from being created, do `{prefix}disable new`.
        To stop all existing threads from DMing Kakashi Mail, do `{prefix}disable all`.
        To check if the DM function for Kakashi Mail is enabled, do `{prefix}isenable`.
        """
    ...
    async def disable_new(self, ctx):
        """
        Stop accepting new Kakashi Mail threads.

        No new threads can be created through DM.
        """
        embed = discord.Embed(
            title="Success",
            description="Kakashi Mail will not create any new threads.",
            color=self.bot.main_color,
        )
        if self.bot.config["dm_disabled"] < 1:
    ...
    async def disable_all(self, ctx):
        """
        Disables all DM functionalities of Kakashi Mail.

        No new threads can be created through DM nor no further DM messages will be relayed.
        """
        embed = discord.Embed(
            title="Success",
            description="Kakashi Mail will not accept any DM messages.",
            color=self.bot.main_color,
        )
    ...
    async def isenable(self, ctx):
        """
        Check if the DM functionalities of Kakashi Mail is enabled.
        """

        if self.bot.config["dm_disabled"] == 1:
            embed = discord.Embed(
                title="New Threads Disabled",
                description="Kakashi Mail is not creating new threads.",
                color=self.bot.error_color,
            )
        elif self.bot.config["dm_disabled"] == 2:
            embed = discord.Embed(
                title="All DM Disabled",
                description="Kakashi Mail is not accepting any DM messages for new and existing threads.",
                color=self.bot.error_color,
            )
        else:
            embed = discord.Embed(
                title="Enabled",
                description="Kakashi Mail now is accepting all DM messages.",
                color=self.bot.main_color,
            )
